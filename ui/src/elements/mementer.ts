...
function Mementer(props: { shadowRoot: any; mementerService: any; route: string; entryHash: string | undefined }) {
    ...
    function saveNewBead() {
      ...
      mementerService
        .createBead({ entryHash: decodeHashFromBase64(entryHash!), bead })
        .then((res: any) => {
          bead.id = encodeHashToBase64(res.entryHash)
    ...
    function updateMementerSettings(newSettings: any) {
      mementerService
        .updateMementer({ entryHash: decodeHashFromBase64(entryHash!), settings: newSettings })
        .then(() => {
    ...
    useEffect(() => {
      if (entryHash && route === 'mementer') {
        Promise.all([
          mementerService.getMementer(decodeHashFromBase64(entryHash)),
          mementerService.getBeads(decodeHashFromBase64(entryHash)),
        ])
          .then((data) => {
            setLoading(false)
            beads.current = data[1].map((item: any) => ({ id: encodeHashToBase64(item.entryHash), ...item.bead }))
